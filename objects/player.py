...
class Player(GameObject):
    def __init__(
        ...
        - HP + pasek życia,
        - dash (LShift),
        - sound waves (LPM) z on-beat + EffectsManager,
        - look-indicator (białe kółko),
        - obsługa kamery (offset rysowania, celowanie),
        - mapa (clamp po map_width/map_height),
        - animacja/„skórka” z assets/pictures/walk_animation.
        """
        ...
        # Bullet-time scale (1.0 = normal) – hook pod zewnętrzny system
        self.time_scale: float = 1.0

        # Sound wave visuals generated on mouse click
        # list of dict: {pos, radius, max_radius, color, thickness, damage, on_beat}
        ...
        # -------------------------- Animacja / skórka -----------------------
        self.anim_frames_right: list[pygame.Surface] = []
        self.anim_frames_left: list[pygame.Surface] = []
        for i in range(25):
            try:
            ...
            # skalowanie zgodnie z self.scale (GameObject)
    ...
    def _handle_input(self) -> None:
        ...
        # Animacja – dokładnie jak w działającym pliku
        if self.anim_frames_right:
            if self.velocity.length_squared() > 0:
                self.anim_index = (self.anim_index + 0.25) % len(self.anim_frames_right)
                if self.facing_right:
                    self.sprite = self.anim_frames_right[int(self.anim_index)]
                else:
                    self.sprite = self.anim_frames_left[int(self.anim_index)]
            else:
                self.sprite = (
                    self.anim_frames_right[0]
                    if self.facing_right
                    else self.anim_frames_left[0]
                )
        ...
        if pressed[0]:  # left click
            if not self._mouse_was_pressed:
                ...
                if now >= self._next_attack_time_ms:
                    ...
                    max_radius = (
                        self.on_beat_max_wave_radius
                        if on_beat_now
                        else self.max_wave_radius
                    )
                    thickness = (
                        self.on_beat_wave_thickness
                        if on_beat_now
                        else self.wave_thickness
                    )
                    ...
                    # EffectsManager – powiadom shader o nowej fali
                    if self.effects_manager is not None and hasattr(
                        self.effects_manager, "add_wave"
                    ):
                        try:
                            self.effects_manager.add_wave(pos)
                        except TypeError:
                            pass
    ...
    def on_beat(self) -> None:
        """Zostawione dla kompatybilności, logika on-beat jest w _handle_mouse_click."""
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        if self.is_dashing:
            base_sprite = self.sprite  # already correctly flipped by anim / facing
            now = pygame.time.get_ticks()
            elapsed = now - self.dash_start_time_ms
            t = max(
                0.0,
            ...
            # -30, -50 jak w działającym pliku (offset na sprite)
            screen.blit(
                self.sprite,
                (self.rect.x - cam_x - 30, self.rect.y - cam_y - 50),
            )

        # Draw sound waves (world -> screen przez kamerę)
        for wave in self.sound_waves:
            if wave["radius"] > 0:
                wx, wy = wave["pos"]
                sx = int(wx - cam_x)
    ...
    def get_health_percentage(self) -> float:
        return 0.0 if self.max_health <= 0 else max(
            0.0, min(1.0, self.current_health / self.max_health)
        )
